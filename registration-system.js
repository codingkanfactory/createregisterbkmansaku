...
function updateNameInFirebase(newName) {
  ...
  const namapemiliknomor = newName.charAt(0).toUpperCase() + currentUID
  ...
  const userUpdate = update(userUpdateRef, {
    name: newName,
    namapemiliknomor: namapemiliknomor, // Update namapemiliknomor when name changes
  })
...
registerForm.addEventListener("submit", (e) => {
  ...
  get(userCheckRef)
    .then((snapshot) => {
      ...
      const namapemiliknomor = nama.charAt(0).toUpperCase() + nisn
